...
2. **Hero.** The one line — *"Keep your tokenized asset compliant for life — every holder provably eligible, continuously re-screened, blocked on-chain the moment they're not. Your firm never touches a single piece of investor PII."* One sharp sub-line. Primary CTA (**See it block a transfer** → demo) + secondary (**Launch app**). Visual: the clean "transfer DENIED on-chain" moment or a restrained product shot. No wall of text — this section is the whole pitch in 5 seconds.
...
4. **How it works (3–4 steps, visual).** Prove privately (ZK, zero uploads) → gated on-chain (every transfer) → continuously re-screened (autonomous agent) → provable to regulators (selective disclosure). A clean horizontal/step diagram, not prose.
...
7. **The trust flex.** *"You can't lie to it."* The fraud-proof angle — every eligibility decision is a published proof anyone can verify, and a false one is slashed on-chain. Short, builds credibility for the technical judges and serious buyers.
...
- **Actions** — freeze / revoke / override, each **multisig-gated** (officer authority), with confirmation + reason.
...
- **Prove eligibility** — step UX: load claims (from KYC issuer) → generate ZK proof client-side → submit proof to the quorum → watch credential status. Clear "your documents never leave your device" reassurance at each step.
...
Backend contracts/agent are tracked separately; the frontend can build against typed mocks for the dashboards until the Credential Registry + agent endpoints land, then wire real data. (Mocks for *frontend scaffolding during parallel dev* only — the shipped demo runs on real on-chain data, no mocks.)
