/*
  ...
  (the one the v3 groth16-verifier checks) client-side via snarkjs — the investor's
  documents never leave the device; only the proof + public inputs are submitted.
...
/** The proven sample witness (a real eligible holder) — lets the demo generate a
    valid proof without re-deriving the issuer signature + Merkle path client-side. */
